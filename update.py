...
Spell = namedtuple('Spell', 'link, name, book, edition, subschool, school, descriptor, levels, verbal, somatic, material, arcanefocus, divinefocus, xpcost, castingtime, spellrange, area, target, duration, savingthrow, spellres, text')

conn = sqlite3.connect('spells.db')
c = conn.cursor()
c.execute("CREATE TABLE spells (id integer primary key, link text, name text, book text, edition text, school, subschool, verbal integer, somatic integer, material integer, arcanefocus integer, divinefocus integer, xpcost integer, castingtime text, spellrange text, area text, target text, duration text, savingthrow text, spellres text, spelltext text);")
